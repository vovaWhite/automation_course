...
matching_cars = [(name, details) for name, details in car_data.items()
                 if details[1] >= year and details[2] >= engine and details[-1] <= price
                 ]
matching_cars_sorted = sorted(matching_cars, key=lambda x: x[1][4])

for car in matching_cars_sorted[:5]:
    name, details = car
    print(f"Car: {name}, Color: {details[0]}, Year: {details[1]}, Engine: {details[2]}L, Type: {details[3]}, Price: ${details[4]}")
